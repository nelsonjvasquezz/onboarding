...
namespace onboarding.persistence.configurations
{
    public class ProgramaConfiguration : IEntityTypeConfiguration<Programa>
    {
        ...
        public void Configure(EntityTypeBuilder<Programa> builder)
        {
            ...
            builder.Property(e => e.FechaModificacion).HasColumnName("pro_fecha_modificacion");

            // Foreign keys
            builder.HasOne(d => d.PlantillaPrograma).WithMany(p => p.Programas).HasForeignKey(d => d.PlantillaProgramaCodigo).OnDelete(DeleteBehavior.NoAction); // FK_obdppr_obdpro
        }
    }
}